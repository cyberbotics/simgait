  ...
  constructor(title, footer, routes) {
    ...
    function setup(project) {
      ...
      project.content.querySelector('#add-a-new-animation').addEventListener('click',
        function(event) { addAnimation(project); });
    ...
    function initSort(project) {
      ...
      document.querySelectorAll('.column-title').forEach((title) => {
        title.addEventListener('click', function(e) {
          ...
          this.sort = sort;
          searchAndSortTable(project);
        });
      });
    }

    function searchAndSortTable(project, isSearch) {
    ...
    function listAnimations(project) {
        ...
        .then(function(data) {
          if (data.error)
            ...
            for (let i = 0; i < data.animations.length; i++)
              line += '<tr>' + animationRow(data.animations[i], project) + '</tr>';
            ...
            for (let i = 0; i < data.animations.length; i++) {
              ...
              if (node) {
                ...
                if (p === 0)
                  p = 1;
                node.addEventListener('click', function(event) { deleteAnimation(event, project, p); });
              }
            }
    ...
    function deleteAnimation(event, project, page) {
    ...
    function animationRow(data, project) {
      let size = data.size;
      let unit;
      if (size < 1024)
        unit = 'bytes';
...
<td><a class="has-text-dark" href="${versionUrl}" target="_blank" title="View Webots release">${data.version}</a></td>`;
    ...
    function initSearch(project, searchString) {
